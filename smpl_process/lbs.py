...
def lbs(betas, pose, v_template, clothed_v_template, shapedirs, posedirs, J_regressor, parents,
    ...
    v_shaped = v_template + blend_shapes(betas, shapedirs) # 根据形状参数调整template vertex位置

    # Get the joints
    # NxJx3 array
    J = vertices2joints(J_regressor, v_shaped) # 根据考虑形状变化的vertex位置计算关节位置
    if clothed_v_template is not None:
        v_shaped = clothed_v_template

    rot_mats = batch_rodrigues(pose.view(-1, 3)).view([batch_size, -1, 3, 3]) # 将轴角表示的姿势参数转换为旋转矩阵

    if posedirs is not None:
        # 3. Add pose blend shapes 根据姿态参数调整vertex位置
        # N x J x 3 x 3
