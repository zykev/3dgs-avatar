...
from smplmodel.lbs import lbs, batch_rodrigues
        ...
        **kwargs) -> None:
        ...
        if self.model_type == 'smplh':
            ...
            for key in ['LEFT', 'RIGHT']:
                left_file = join(os.path.dirname(model_path), 'MANO_{}.pkl'.format(key))
...
def load_model(gender='neutral', use_cuda=True, model_type='smpl', skel_type='body25', device=None, model_path='data/body_models'):
    # prepare SMPL model
    ...
    if model_type == 'smpl':
        ...
        body_model = SMPLlayer(os.path.join(model_path, 'smpl', gender, 'model.pkl'), gender=gender, device=device,
            regressor_path=None)
    else:
        body_model = None
