...
class CPB(nn.Module):
    ...
    def forward(self, grid_q, grid_kv):
        ...
        pos = rearrange(grid_q, 'b i c -> b i 1 c') - rearrange(grid_kv, 'b j c -> b 1 j c') # 计算grid_q和grid_kv之间的位置偏移量
        bias = torch.sign(pos) * torch.log(pos.abs() + 1)  # log of distance is sign(rel_pos) * log(abs(rel_pos) + 1)

        for layer in self.mlp:
            bias = layer(bias)

        bias = rearrange(bias, '(b g) i j o -> b (g o) i j', g = self.offset_groups) # [1, 8, 4096, 256]
...
class DeformableAttention2D(nn.Module):
    ...
    def forward(self, x, return_vgrid = False):
        ...
        q = self.to_q(x) # [1, 512, 64, 64]

        # calculate offsets - offset MLP shared across all groups

        group = lambda t: rearrange(t, 'b (g d) ... -> (b g) d ...', g = self.offset_groups)

        grouped_queries = group(q) # [8, 64, 64, 64]
        offsets = self.to_offsets(grouped_queries) # [8, 2, 16, 16] # 每个reference point有offset_groups个offset
        ...
        vgrid_scaled = normalize_grid(vgrid) # [8, 16, 16, 2] 每个reference point加上offset后的坐标

        kv_feats = F.grid_sample(
            group(x),
            vgrid_scaled,
        mode = 'bilinear', padding_mode = 'zeros', align_corners = False) # [8, 64, 16, 16] 每个reference point加上offset后对应的特征

        kv_feats = rearrange(kv_feats, '(b g) d ... -> b (g d) ...', b = b) # [1, 512, 16, 16] 每个reference point aggregate所有offset的特征之后的特征
        ...
        q, k, v = map(lambda t: rearrange(t, 'b (h d) ... -> b h (...) d', h = heads), (q, k, v)) # [bs, number of heads, number of features for each head (h*w), feature dimension]
