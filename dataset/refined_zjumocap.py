...
from utils.dataset_utils import preprocess_image
...
class RefinedZJUMoCapDataset(Dataset):
    def __init__(self, cfg, split='train'):
        ...
        frames = list(range(len(os.listdir(os.path.join(self.subject_dir, 'smpl_params')))))
        ...
        for cam_idx, cam_name in enumerate(cam_names):
            img_files = sorted(glob.glob(os.path.join(self.subject_dir, f'images/{cam_name}', '*.jpg')))[frame_slice]
            # mask_files = sorted(glob.glob(os.path.join(self.subject_dir, f'mask/{cam_name}', '*.png')))[frame_slice]
    ...
    def getitem(self, idx, data_dict=None):
        ...
        image, mask, semantic_feature, seg_label, depth, normal = preprocess_image(img_file, K, D, (self.w, self.h), self.white_bg, self.cfg.get('pca_components', 48))
