...
from smpl_loader.lbs import (
    lbs, vertices2landmarks, find_dynamic_lmk_idx_and_bcoords, blend_shapes)

from smpl_loader.vertex_ids import vertex_ids as VERTEX_IDS
from smpl_loader.utils import (
...
from smpl_loader.vertex_joint_selector import VertexJointSelector
...
class SMPL(nn.Module):
    ...
    def __init__(
        ...
        **kwargs
        ...
        parents[0] = -1
        self.register_buffer('kintree_parents', parents)
    ...
    def forward(
        ...
        **kwargs
        ...
        vertices, joints, transforms_mat = lbs(betas, full_pose, self.v_template,
                               self.shapedirs, self.posedirs,
                               self.J_regressor, self.kintree_parents,
...
class SMPLLayer(SMPL):
    ...
    def forward(
        ...
        **kwargs
        ...
        # ones from the module
        global_orient = (global_orient if global_orient is not None else
                         torch.zeros([self.batch_size, 3]))

        body_pose = body_pose if body_pose is not None else torch.zeros([self.batch_size, self.NUM_BODY_JOINTS * 3])
        betas = betas if betas is not None else torch.zeros([self.batch_size, self.num_betas])

        transl = transl if transl is not None else torch.zeros([self.batch_size, 3])
        # apply_trans = transl is not None or hasattr(self, 'transl')
        # if transl is None and hasattr(self, 'transl'):
        #     transl = self.transl
        ...
        if full_pose.shape[0] != batch_size:
            ...
            full_pose = full_pose.expand(num_repeats, -1)
